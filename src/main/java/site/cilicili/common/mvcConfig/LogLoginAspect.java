...
public class LogLoginAspect {
    ...
    @Pointcut("execution(* site.cilicili.authentication.user.controller.UsersController.logout(..)) || execution(* site.cilicili.backend.user.controller.SysUserOnlineController.kickOnlineUser(..))")
    ...
    public void logLogin(JoinPoint joinPoint, R res) {
        Optional.ofNullable(res).flatMap(r -> Optional.ofNullable(joinPoint.getArgs())).ifPresent(objects -> {
            ...
            final SysLogLoginDto.SysLogLoginDtoBuilder builder = SysLogLoginDto.builder()
                    ...
                    .loginBrowser(String.format("%1$s %2$s", userAgent.getBrowser().getName(), userAgent.getVersion()))
                    .loginUsername(user.getUsername())
                    .stable("yesNo_no")
                    .status("onOff_on")
                    .memo(String.format("%1$s %2$s", res.getMessage(), agentString))
                    .sort(1L)
                    .loginIp(Objects.requireNonNull(httpHeaders.getHost()).getHostString())
                    .createdAt(LocalDateTime.now());
            if (res.getCode() == 1) {
                builder.loginSuccess("yesNo_yes");
                stringRedisTemplate.opsForValue().setIfAbsent(user.getUsername(), JSONUtil.toJsonStr(builder.build(), JSON_CONFIG));
            } else {
                builder.loginSuccess("yesNo_no");
                sysLogLoginService.insert(BeanUtil.toBean(builder.build(), SysLogLoginEntity.class));
            }
            if (logger.isDebugEnabled()) {
                logger.info("{} IP:{}-->{}", user.getUsername(), httpHeaders.getHost(), res);
            }
        });
    }
        ...
        Optional.ofNullable(joinPoint.getArgs())
                ...
                .map(o -> {
                    ...
                    if (o instanceof KickOnlineUserRequest kickOnlineUserRequest) {
                        return Optional.ofNullable(sysUserService.getOne(new QueryWrapper<SysUserEntity>().eq("username", kickOnlineUserRequest.username())))
                                .map(sysUserEntity -> AuthUserDetails.builder().username(sysUserEntity.getUsername()).realName(sysUserEntity.getRealName()).id(sysUserEntity.getId()).build())
                                .orElse(AuthUserDetails.builder().username(kickOnlineUserRequest.username()).realName(kickOnlineUserRequest.username()).build());
                    }
                    return null;
                })
                .filter(authUserDetails -> stringRedisTemplate.hasKey(authUserDetails.getUsername()))
                .map(authUserDetails -> {
                    final SysLogLoginEntity sysLogLoginEntity = JSONUtil.toBean(stringRedisTemplate.opsForValue().get(authUserDetails.getUsername()), JSON_CONFIG, SysLogLoginEntity.class);
