...
public class ChartServiceImpl implements ChartService {
    ...
    public R getCiliDataBoard() {
        ...
        final List<CiliDataBoardResponse.Main4> main4 = Optional.ofNullable(memberShipMapper.list(new QueryWrapper<>()))
                .map(memberShipEntities1 -> BeanUtil.copyToList(memberShipEntities1, CiliDataBoardResponse.Main4.class)).orElse(Collections.emptyList());
        ...
        return R.yes("Success.").setData(CiliDataBoardResponse.builder()
                .main1(main1)
                .main4(main4)
                .main3(main3)
                .main5(CiliDataBoardResponse.Main5.builder()
                        .yData(main5.stream().map(item -> item.get("vis")).toList())
                        .xData(main5.stream().map(item -> item.get("name")).toList())
                        .build())
                .main6(CiliDataBoardResponse.Main6.builder().uploadData(main6).build())
                .build());
    }
    ...
    private CiliDataBoardResponse.Main3 getMain2(final Date end, final DateTime start, final List<String> dateTimes) {
        ...
        final List<Long> totalByWeek = dateTimes.stream().map(item -> {
        ...
        final List<Long> onByWeek = dateTimes.stream().map(item -> {
        ...
        final List<Long> passByWeek = dateTimes.stream().map(item -> {
        ...
        titles.add(CiliDataBoardBackendResponse.Main2.Title.builder()
                .name("数据总量").value(totalByWeek.stream().reduce(0L, Long::sum))
                .build());
        titles.add(CiliDataBoardBackendResponse.Main2.Title.builder()
                .name("投稿量").value(onByWeek.stream().reduce(0L, Long::sum))
                .build());
        titles.add(CiliDataBoardBackendResponse.Main2.Title.builder()
                .name("通过量").value(passByWeek.stream().reduce(0L, Long::sum))
                .build());

        return CiliDataBoardResponse.Main3.builder()
                .xData(dateTimes)
                .yData(CiliDataBoardResponse.Main3.YData.builder()
                        .updateData(totalByWeek.stream().map(CiliDataBoardResponse.Main3.YData.Value::new).toList())
                        .viewData(onByWeek.stream().map(CiliDataBoardResponse.Main3.YData.Value::new).toList())
                        .uploadData(passByWeek.stream().map(CiliDataBoardResponse.Main3.YData.Value::new).toList())
                        .build())
                .title(titles)
                .build();
    }
