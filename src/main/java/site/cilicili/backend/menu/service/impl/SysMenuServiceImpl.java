...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenuEntity> implements SysMenuService {
    ...
    public R addMenu(final AuthUserDetails authUserDetails, final AddMenuRequest addMenuRequest) {
        return Optional.of(saveOrUpdate(BeanUtil.toBean(addMenuRequest, SysMenuEntity.class))).filter(flag -> flag).map(flag -> R.yes("添加成功.")).orElseThrow(() -> new AppException(Error.COMMON_EXCEPTION));
    }
    ...
    public R editMenu(final AuthUserDetails authUserDetails, final SysMenuDto.EditMenuRequest addMenuRequest) {
        return Optional.ofNullable(authUserDetails).filter(auth -> Objects.nonNull(getById(addMenuRequest.getId())) && ((Objects.nonNull(addMenuRequest.getButton()) && !addMenuRequest.getButton().isEmpty() && sysButtonService.insertOrUpdate(addMenuRequest.getButton())) | updateById(addMenuRequest))).map(auth -> R.yes(String.format("%1$s编辑成功.", auth.getusername()))).orElseThrow(() -> new AppException(Error.COMMON_EXCEPTION));
    }
    ...
    public R deleteMenuById(final AuthUserDetails authUserDetails, final FindOrDeleteRequest findOrDeleteRequest) {
        return Optional.ofNullable(authUserDetails).map(auth -> baseMapper.selectById(findOrDeleteRequest.id()))
                .filter(sysMenuEntity -> !"yesNo_yes".equals(sysMenuEntity.getStable()))
                .filter(sysMenuEntity -> sysButtonService.remove(new QueryWrapper<SysButtonEntity>().eq("menu_name", sysMenuEntity.getName())) | remove(new QueryWrapper<SysMenuEntity>().eq("parent_code", sysMenuEntity.getName())) | removeById(sysMenuEntity))
