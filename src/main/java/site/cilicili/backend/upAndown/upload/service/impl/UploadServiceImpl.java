...
public class UploadServiceImpl implements UploadService {
    ...
    public R uploadUserAvatar(final AuthUserDetails authUserDetails, final MultipartFile[] multipartFile) {
        return Optional.ofNullable(multipartFile).filter(multipartFiles -> multipartFiles.length > 0).filter(multipartFiles -> {
            final String suffix = Optional.ofNullable(sysConfigBackendService.getBaseMapper().selectOne(new QueryWrapper<SysConfigBackendEntity>().eq(BackendConfigItem.AVATAREXT.getKey(), BackendConfigItem.AVATAREXT.getItem()))).map(sysConfigBackendEntity -> Optional.ofNullable(sysConfigBackendEntity.getItemCustom()).filter(StrUtil::isNotBlank).orElse(sysConfigBackendEntity.getItemDefault())).orElseThrow(() -> new AppException(Error.COMMON_EXCEPTION));
            return multipartFiles.length == Arrays.stream(multipartFiles).filter(item -> suffix.startsWith(Objects.requireNonNull(item.getOriginalFilename()).substring(item.getOriginalFilename().indexOf("."))) || suffix.endsWith(item.getOriginalFilename().substring(item.getOriginalFilename().indexOf(".")))).count();
        }).map(multipartFiles -> {
            final String location = Optional.ofNullable(sysConfigBackendService.getBaseMapper().selectOne(new QueryWrapper<SysConfigBackendEntity>().eq(BackendConfigItem.UPLOADAVATARSAVEPATH.getKey(), BackendConfigItem.UPLOADAVATARSAVEPATH.getItem()))).map(sysConfigBackendEntity -> Optional.ofNullable(sysConfigBackendEntity.getItemCustom()).filter(StrUtil::isNotBlank).orElse(sysConfigBackendEntity.getItemDefault())).orElseThrow(() -> new AppException(Error.COMMON_EXCEPTION));
            return Optional.ofNullable(sysUserService.getBaseMapper().selectById(authUserDetails.getId())).map(userEntity -> {
                final List<String> list1 = Arrays.stream(multipartFiles).map(item -> {
                    try {
                        String loc = "%1$s/%2$s/%3$s", prefix = "gqa-upload:";
                        final String avatar = String.format(loc, location, authUserDetails.getusername(), String.format("%1$s_%2$s%3$s", IdUtil.fastSimpleUUID(), LocalDateTimeUtil.format(LocalDateTime.now(), "yyyyMMddHHmmssSSS"), Objects.requireNonNull(item.getOriginalFilename()).substring(item.getOriginalFilename().indexOf("."))));
                        String loc2 = prefix + avatar;
                        Optional.ofNullable(userEntity.getAvatar()).filter(StrUtil::isNotBlank).flatMap(av -> Optional.of(new File(av.replace(prefix, ""))).filter(File::exists)).ifPresent(FileUtil::del);
