<template>
  <q-dialog v-model="recordDetailVisible" position="top">
    <q-card style="width: 800px; max-width: 80vw;">
      <q-card-section class="text-h6">
        {{ formTypeName }} {{ $t('Banner') }}:
        {{ recordDetail.value.image_url }}
      </q-card-section>
      <q-separator/>
      <q-card-section>
        <q-form ref="recordDetailForm">
          ...
          <div class="row q-gutter-md">
            ...
            <q-input v-model="recordDetail.value.link_path" :label="$t('Link') + $t('Address')"
                     :rules="[val => val && val.length > 0 || $t('NeedInput')]" class="col"
                     hint=""
                     outlined/>
            <q-input v-model="recordDetail.value.link_page" :label="$t('Jump') + $t('Page')"
                     :rules="[val => val && val.length > 0 || $t('NeedInput')]" class="col"
                     hint=""
                     outlined/>
          ...
          <div class="row q-gutter-md">
            <q-select v-model="recordDetail.value.jump_type" :label="$t('Jump') + $t('Type')"
                      :options="dictOptions.jumpType" :rules="[val => val && val.length > 0 || $t('NeedInput')]"
                      class="col" emit-value hint=""
                      map-options
                      outlined/>
            <q-input v-model="recordDetail.value.memo" :label="$t('Memo')" class="col" hint="" outlined/>
          </div>
          <CiliUpload :multiple="false" title="轮播图上传" @setUrl="setUrl"/>
  ...
  <q-dialog v-model="recordDetailVisible2">
    <q-card style="width: 800px; max-width: 80vw;">
      <n-carousel autoplay draggable interval="2500">
        <img
          v-for="(item,index) in recordDetail.value" :key="item.sort"
          :class="$q.screen.gt.sm?'carousel-img':'carousel-img2'"
          :src="item.url"
        >
      </n-carousel>
    </q-card>
...
const url = {
  ...
  queryById: 'carousel/query-carousel-by-id',
}
const setUrl = (url) => {
  recordDetail.value.image_url = url
}
const {
  dictOptions,
  formType,
