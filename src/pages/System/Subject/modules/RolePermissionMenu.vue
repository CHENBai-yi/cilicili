<template>
  <div class="items-center column">
    ...
    <q-card-section class="scroll" style="width: 100%; max-height: 70vh">
      <q-tree v-if="menuTree.length !== 0" v-model:ticked="ticked" :nodes="menuTree" default-expand-all dense
              label-key="name"
              node-key="name" selected-color="primary" style="width: 100%" tick-strategy="strict">
        <template v-slot:default-header="prop">
          <div class="items-center row">
            <q-icon :name="prop.node.icon || 'share'" class="q-mr-sm" size="20px"/>
            <div class="text-weight-bold">
              {{ selectOptionLabel(prop.node) }}
            </div>
            <q-radio v-model="defaultPage" :val="prop.node.name" dense>
              <q-tooltip anchor="center end" self="center left">
                {{ $t('Default') + $t('Page') }}
              </q-tooltip>
            </q-radio>
            <div class="row q-gutter-x-md" style="margin-left: 10px">
              <q-checkbox v-for="item in prop.node.button" v-model="buttonCheckMap[item.button_code]"
                          :false-value="null"
                          :label="item.button_name" color="primary" dense/>
            </div>
          </div>
        </template>
