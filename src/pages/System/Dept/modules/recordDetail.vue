    ...
    <q-card style="width: 1200px; max-width: 80vw;">
      ...
      <div class="row">
          ...
          <q-form ref="recordDetailForm">
            <div class="row q-gutter-md">
              <q-input v-model="recordDetail.value.dept_code" :disable="formType === 'edit'"
                       :label="$t('Dept') + $t('Code')" :rules="[val => val && val.length > 0 || $t('NeedInput')]"
                       class="col" hint=""
                       outlined/>
              <q-input v-model="recordDetail.value.dept_name" :label="$t('Dept') + $t('Name')"
                       :rules="[val => val && val.length > 0 || $t('NeedInput')]" class="col"
                       hint=""
                       outlined/>
              <q-input v-model="recordDetail.value.parent_code" :label="$t('Parent') + $t('Dept') + $t('Code')"
                       class="col" hint=""
                       outlined readonly/>
            </div>
            <div class="row q-gutter-md">
        ...
        <q-card-section class="q-gutter-md col">
          <q-field :disable="!recordDetail.value.parent_code" :label="$t('Parent') + $t('Dept')" dense hint="" outlined
                   stack-label>
            <template v-slot:control>
              <q-tree v-model:selected="recordDetail.value.parent_code" :nodes="deptTree" default-expand-all
                      label-key="dept_name" node-key="dept_code" selected-color="primary" style="width: 100%;"
