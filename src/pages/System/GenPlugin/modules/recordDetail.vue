<template>
  <q-dialog v-model="recordDetailVisible" persistent position="top">
    <q-card style="width: 1500px; max-width: 90vw;">
      ...
      <q-card-section>
        <q-form ref="recordDetailForm">
          ...
          <div class="row q-gutter-md">
            ...
            <q-input v-model.trim="form.plugin_code" :label="$t('Plugin') + $t('Code')"
                     :rules="[val => val && val.length > 0 || $t('NeedInput')]" class="col" dense
                     hint="Camelcase" outlined
                     prefix="plugin-"/>
            <q-input v-model.trim="form.plugin_name" :label="$t('Plugin') + $t('Name')"
                     :rules="[val => val && val.length > 0 || $t('NeedInput')]" class="col" dense
                     hint="" outlined/>
            <q-input v-model.trim="inputModel" :label="$t('Add') + 'Model'" class="col" dense hint="Camelcase"
                     outlined>
              <template v-slot:after>
                <q-btn dense icon="add" round @click="handleInputModel"/>
              </template>
            </q-input>
          </div>

          <div v-if="form.plugin_model.length" class="row q-gutter-md" style="margin-top: 10px;">
            <q-card style="width: 100%">
              <q-tabs v-model="modelStruct" align="left" inline-label mobile-arrows outside-arrows>
              ...
              <q-tab-panels v-model="modelStruct" animated style="width: 100%">
                <q-tab-panel v-for="(item, index) in form.plugin_model" :name="item.model_name"
                             style="padding: 0;">
                  <q-table :columns="columns" :rows="item.column_list" hide-pagination
                           row-key="column_name">
                    ...
                    <template v-slot:body-cell-column_name="props">
                      <q-td :props="props">
                        ...
                        <q-popup-edit v-slot="scope" v-model="props.row.column_name">
                          <q-input v-model.trim="scope.value" autofocus dense
                    ...
                    <template v-slot:body-cell-column_type="props">
                      <q-td :props="props">
                        ...
                        <q-popup-edit v-slot="scope" v-model="props.row.column_type">
                    ...
                    <template v-slot:body-cell-column_comment="props">
                      <q-td :props="props">
                        ...
                        <q-popup-edit v-slot="scope" v-model="props.row.column_comment">
                          <q-input v-model.trim="scope.value" autofocus dense
                                   @keyup.enter="scope.set"/>
                        </q-popup-edit>
                      </q-td>
                    ...
                    <template v-slot:body-cell-column_default="props">
                      <q-td :props="props">
                        ...
                        <q-popup-edit v-slot="scope" v-model="props.row.column_default">
                          <q-input v-model.trim="scope.value" autofocus dense
                                   @keyup.enter="scope.set"/>
                        </q-popup-edit>
                      </q-td>
                    ...
                    <template v-slot:body-cell-actions="props">
                      <q-td :props="props">
                        <div class="q-gutter-xs">
                          <q-btn color="negative" label="移除"
                                 @click="removeColumn(item, props.row)"/>
                        </div>
                      </q-td>
                    </template>
                  </q-table>
      ...
      <q-card-actions align="right">
        <q-btn v-has="'genPlugin:gen'" :label="$t('Gen')" color="primary" @click="handleAddOrEidt"/>
