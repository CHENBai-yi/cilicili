...
const handleChangePasswrod = async () => {
  ...
  if (success) {
    if (passwordForm.value.new_password_1 !== passwordForm.value.new_password_2) {
      window.$message.error(t('TwoPasswordsError'), {render: window.$render})
    } else {
      const res = await postAction(changePasswordUrl, passwordForm.value)
      if (res.code === 1) {
        await userStore.HandleUserLogout()
        window.$message.success(t('Change') + ' ' + t('Password') + ' ' + t('Success') + ',' + t('Relogin'), {render: window.$render})
    ...
    window.$message.error(t('FixForm'), {render: window.$render})
  }
}
</script>
